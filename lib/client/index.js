...
client.load = function load (serverSettings, callback) {
  ...
  function brushed () {
    ...
    function updateCurrentSGV (entry) {
      var value = entry.recalculated ? entry.recalculated : entry.mgdl
  ...
  */
  ...
  function prepareEntries () {
    ...
    var temp2 = client.ddata.sgvs.map(function(obj) {
      return { mills: obj.mills, mgdl: obj.mgdl, direction: obj.direction, color: sgvToColor(obj.mgdl), type: 'sgv', noise: obj.noise, filtered: obj.filtered, unfiltered: obj.unfiltered, recalculated: obj.recalculated };
    });
    var temp3 = client.ddata.sgvs.map(function(obj) {
      if (obj.recalculated && obj.recalculated !== obj.mgdl) {
        return { mills: obj.mills, mgdl: obj.recalculated, direction: obj.direction, color: sgvToColor(obj.recalculated), type: 'recalc', noise: obj.noise, filtered: obj.filtered, unfiltered: obj.unfiltered };
      } else {
        return null;
      }
    }).filter(function(entry) {
    ...
    client.entries = [];
    client.entries = client.entries.concat(temp1, temp2, temp3);
