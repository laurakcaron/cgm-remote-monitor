...
function init (client, d3) {
  ...
  renderer.addFocusCircles = function addFocusCircles () {
    ...
    function prepareFocusCircles (sel) {
      updateFocusCircles(sel)
        .attr('fill', function(d) {
          return d.type === 'forecast' ? 'none' : d.type === 'sgv' ? 'none' : d.color;
        })
        .attr('stroke-opacity', function(d) {
          return d.type === 'sgv' ? .3 : 1;
        })
        .attr('stroke-width', function(d) {
          return d.type === 'mbg' ? 2 : d.type === 'forecast' ? 2 : d.type === 'sgv' ? 1.5 : 0 ;
        })
    ...
    function focusCircleTooltip (d) {
      if (d.type !== 'sgv' && d.type !== 'mbg' && d.type !== 'forecast' && d.type !== `recalc`) {
        return;
      }
      ...
      function getRawbgInfo () {
        ...
        if (d.type === 'recalc') {
          info.noise = client.rawbg.noiseCodeToDisplay(d.recalculated, d.noise);
          if (client.rawbg.showRawBGs(d.recalculated, d.noise, client.ddata.cal, sbx)) {
            info.value = utils.scaleMgdl(client.rawbg.calc(d, client.ddata.cal, sbx));
          }
        }
        return info;
      ...
      client.tooltip.html('<strong>' + translate('BG') + ':</strong> ' + client.sbx.scaleEntry(d) +
          ...
          '<br/><strong>' + translate('Time') + ':</strong> ' + client.formatTime(getOrAddDate(d)) + (d.type === 'recalc' ? '<br/><strong>Type: </strong> Smoothed' : '' ))
